    ...
    <div className="dash">
      <header>
        ...
        <div className="stats">
          ...
          <span>
            last pulled{' '}
            <span className={'wh-emoji' + (isPulling ? ' spinning' : '')}>🌀</span>{' '}
      ...
      {view === 'journeys' && (
        <JourneyList journeys={snap.journeys} onSelect={setActiveTopic} />
      )}

      {view === 'highlights' && (
        <Highlights highlights={snap.highlights} onChange={() => void loadSnapshot().then(setSnap)} />
      )}
                      ...
                      return (
                        <div key={subKey} className="sub">
                          <button
                            ...
                            onClick={() => setExpanded((p) => ({ ...p, [subKey]: !subOpen }))}
                          >
                          ...
                          {subOpen && (
                            <ul className="pages">
                              {visible.map((p) => (
                                <li key={p.url + p.visitedAt}>
                                  <a href={p.url} target="_blank" rel="noreferrer" title={p.url}>
                                    {p.title || p.url}
                                  </a>
                                  <span className="when">{formatTime(p.visitedAt)}</span>
                                </li>
                              ))}
